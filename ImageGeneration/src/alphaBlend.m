function [C,RC] = alphaBlend(varargin)
...
if (~isempty(options.ColorChannels) && ischar(options.ColorChannels))
    switch (options.ColorChannels)
     case 'green-magenta'
        options.ColorChannels = [2 1 2];
     case 'red-cyan'
        options.ColorChannels = [1 2 2];
    end
end
...
parser.addParamValue('Scaling', 'independent', @checkScaling);
parser.addParamValue('ColorChannels', '', @checkColorChannels);
...
options.Scaling = lower(parser.Results.Scaling);
options.ColorChannels = lower(parser.Results.ColorChannels);
A = parser.Results.A;
B = parser.Results.B;
...
if isSpatiallyReferencedSyntax
    ...
    function TF = checkScaling(user_param)
        
        valid_scalings = {'joint','independent','none'};
        
        if ~ischar(user_param)
            error(message('images:imfuse:invalidScalingValue'));
        end
        
        % look for the parameter amongst the possible values
        logical_idx = strcmpi(user_param, valid_scalings);
        if ~any(logical_idx)
            error(message('images:imfuse:invalidScalingValue'));
        end
    ...
    function TF = checkMethod(methodArg)
        ...
        valid_methods = {'falsecolor','diff','blend','montage'};
    ...
    function TF = checkColorChannels(arg)
        ...
        if (isnumeric(arg))
            ...
            validatestring(arg, {'red-cyan', 'green-magenta'}, mfilename, 'ColorChannels');
